...
**A2b@20 已闭合主路径 latent 栈选型**（`ACC_param`=0.9380）。**A2c@100 已闭合 mild 辅助续训**（`ACC_param`=0.9564，`parallel`=0.8428，仍低于 DeepCADOrigin@1000 的 0.8617）。当前主线为 **A2d（A2b 栈从头 + hard BCE 300 epoch）**：P2d-01/02 功能改造与 smoke 已通过，**P2d-03 性能优化已落地**（2026-05-24），**长训可启动**；并行可推进 A3/A4 结构单因素消融。
...
| **A2d hard BCE 300** | **可启动**（P2d-01/02 已通过；P2d-03 优化 **2026-05-24**） | -- | -- | **A2b 栈从头训练**；interpreter 起点修正 + `geom_loss` 双向 hard BCE + `γ` 四路拆分（`γ_para=3`）+ 三段调度（1–50 CAD-only / 51–90 warmup / 91–300 full）；默认 `save_frequency=10`；目标 `parallel ≥ 0.86` |
...
| A2d | `cf_high_modify_ablate_dimz256_a2d_hard_bce_300` | 阶段 2d：**A2b 栈从头 + 几何 hard BCE 改造**（**非 CAD-only**） | 结构同 A2b；**从头训练 `nr_epochs=300`**；interpreter 起点修正 + `geom_loss` 双向 hard BCE + `gamma` 四路拆分（`γ_para=3`）+ 三段调度（1–50 CAD-only / 51–90 warmup / 91–300 full）；**P2d-03 向量化已实现** | 破解 A2c `parallel` 0.84 平台；目标越过 DeepCADOrigin@1000 的 `parallel`=0.8617 |
...
**验证**：`verify_p2d.py` `checks_passed=true`；GPU 微基准 `_bench_a2d.py`（B=64, L=60）旧 **3313 ms/iter** → 新 **8.5 ms/iter**（**~388×**）。单 epoch 预期回到 **A2c 同量级**；300 epoch 默认 periodic ckpt **30 个 + latest**（约 **6 GB**）。详见 `主路径消融定位实验记录模板.md` §6.6。
...
`differentiable_sketch_interpreter.py`（**P2d-03 向量化**，替代原 Python 循环草案）：
...
| A2d corrected interpreter Python 循环导致单 epoch 极慢 | 300 epoch 长训不可排期 | **P2d-03 已修复**：向量化 + BCE 合批 + metric 懒求值；微基准 ~388×；见 §3.2d(5) |
