...
**A2b@20 已闭合主路径 latent 栈选型**（`ACC_param`=0.9380）。**A2c@100 已闭合 mild 辅助续训**（`ACC_param`=0.9564，`parallel`=0.8428，仍低于 DeepCADOrigin@1000 的 0.8617）。当前主线为 **A2d（A2b 栈从头 + hard BCE 300 epoch）**：P2d-01/02/02b 功能改造、性能优化与 dataset cache 已通过（2026-05-24），**长训可启动**（须 **`--dataset_cache disk`**、`batch_size=256`、`num_workers=0`）；并行可推进 A3/A4 结构单因素消融。
...
| **A2d hard BCE 300** | **可启动**（P2d-01/02/02b 已通过；dataset cache **2026-05-24**） | -- | -- | **A2b 栈从头训练**；interpreter 修正 + hard BCE + `γ` 四路 + 三段调度；**`--dataset_cache disk`**；`batch_size=256` `num_workers=0`；默认 `save_frequency=10`；目标 `parallel ≥ 0.86` |
...
| A2d | `cf_high_modify_ablate_dimz256_a2d_hard_bce_300` | 阶段 2d：**A2b 栈从头 + 几何 hard BCE 改造**（**非 CAD-only**） | 结构同 A2b；**从头训练 `nr_epochs=300`**；interpreter 起点修正 + `geom_loss` 双向 hard BCE + `gamma` 四路拆分（`γ_para=3`）+ 三段调度（1–50 CAD-only / 51–90 warmup / 91–300 full）；**P2d-02b 向量化**；**`--dataset_cache disk`** + **`batch_size=256` `num_workers=0`** | 破解 A2c `parallel` 0.84 平台；目标越过 DeepCADOrigin@1000 的 `parallel`=0.8617 |
...
| **阶段 2d** | **A2d（A2b 栈从头 + interpreter 修正 + hard BCE + γ 四路拆分 + 300 epoch）** | A2c 闭合后做：从头训练 300 epoch；**P2d-01~03 已落地**，长训可启动；以 **A2c@100** 为对照基线 |
...
- **向量化实现（P2d-03，已落地）**：不再使用 per-batch / per-line Python 循环。对全序列 `(B, L)` 用 segment-aware **`cummax`** 一次性求每个 token 的「同 SOL 段内上一条 curve 位置」；`start/end/unit` 经 fancy index scatter 到 `(B, max_lines, …)`。legacy 分支（`use_corrected_line_start=False`）同样向量化。
- 跨 `SOL_IDX` 边界时不取上一条 sketch 的 curve，而是用 `(0, 0)` 当起点
- 引入 `--use_corrected_line_start` 开关，**默认 `False`**，A2d 必须开启；旧实验复跑保持 `False` 以保留可比性
- Smoke：`verify_p2d.py` **`checks_passed=true`**；合成两 line 序列 `unit_err=0`

##### (2) `geom_loss`：soft 残差 → 双向 hard BCE

[`geometry_constraint.py`](constraint_fused_deepcad_simplify_modify2_low_riskbased_high_modify/application/geometry_constraint.py) hard BCE 路径（P2d-01/03）：unary 两分量与 pair 两分量各 **stack 后 1 次 BCE**（4→2 次）；`pos_weight` 懒缓存，不污染旧 checkpoint 的 `state_dict`。
...
##### (5) 训练性能优化（P2d-03，2026-05-24 已落地）

初版 A2d 实现中 `use_corrected_line_start` 使用 Python 三层循环（batch × line × scan），每步含 `.item()`/`.tolist()` GPU 同步，导致**单 epoch 耗时远超 A2c**，300 epoch 长训不可排期。

| 项 | 文件 | 改动 |
| --- | --- | --- |
| A | `differentiable_sketch_interpreter.py` | corrected / legacy 全向量化；segment-aware `cummax` + scatter |
| B | `geometry_constraint.py` | unary/pair 各 1 次 BCE 合批；`pos_weight` 懒缓存 |
| C | `train_use_case.py` + `train.py` | `compute_metrics` 开关；`geom_*` 与 `.item()` 仅在 `log_frequency` 步 |
| D | `config_constraint_fused_high_modify.py` | `--save_frequency` 默认 **5→10** |
| E | `infrastructure/dataset_high_modify.py` + `dataset_cache_high_modify.py` | **`--dataset_cache disk`**：约束 GT 落盘 `data/.cache/high_modify/{key}/{phase}/`；epoch 2 起跳过重复 CPU 约束提取；`--continue` 可复用 |

**验证**：`verify_p2d.py` `checks_passed=true`；GPU 微基准 `_bench_a2d.py`（B=64, L=60）旧 **3313 ms/iter** → 新 **8.5 ms/iter**（**~388×**）。单 epoch GPU 步与 A2c 同量级；**须 `--dataset_cache disk`** 避免 300 epoch 重复 CPU 预处理（epoch 1 建 cache，epoch 2+ 显著加速）。300 epoch 默认 periodic ckpt **30 个 + latest**（约 **6 GB**）。详见 `主路径消融定位实验记录模板.md` §6.6。
...
parser.add_argument("--dataset_cache", type=str, default="off",
                    choices=["off", "memory", "disk"],
                    help="A2d 长训须 disk：约束 GT 落盘 data/.cache/high_modify/")
parser.add_argument("--dataset_cache_dir", type=str, default="",
                    help="可选 cache 根目录 override")
```

`differentiable_sketch_interpreter.py`（**P2d-02b 向量化**，替代原 Python 循环草案）：

```python
# segment-aware cummax：prev_curve_pos[b, pos] = 同 SOL 段内 pos 之前最近 curve 的 index
curve_mask = (commands.unsqueeze(-1) == curve_cmd_ids).any(dim=-1)
cand_shifted = torch.cat([pad_neg1, curve_idx[:, :-1]], dim=1)
prev_curve_pos = cand_shifted.cummax(dim=1).values
valid_prev = prev_curve_pos > prev_sol_pos.cummax(dim=1).values
start_per_token = end_per_token.gather(1, prev_safe.unsqueeze(-1).expand(B, L, 2))
start_per_token = start_per_token * valid_prev.unsqueeze(-1).to(dtype)
unit_per_token = (end_per_token - start_per_token) / norm.clamp_min(eps)
# scatter line_cmd_mask 命中位置 → (B, max_lines, 2)
```
...
- **须传** `--dataset_cache disk`：161k 训练样本的约束 GT 由 `cad_vec` 确定性导出，仅需提取一次；cache 写入 `data/.cache/high_modify/{key}/train/`（key 由 `angle_thresh/dist_thresh/grid_size/max_lines/...` 哈希）；epoch 1 边训边建 cache，epoch 2 起 CPU 数据加载显著加速；`--continue` 重启训练可复用已有 cache。
- **`batch_size=256`、`num_workers=0`**：A10（22GB）上 A2d 直接 batch 512 OOM；256 为当前 GPU 稳定上限（630 step/epoch，与 OriginDeepCAD batch 512 的 315 step 不同，有效 batch 减半）。
...
- **默认 `save_frequency=10`**（2026-05-24 起仓库默认；300 epoch 约 **30** 个 periodic ckpt + `latest`，磁盘约 **6 GB**）。若需更密过渡区监控可显式 `--save_frequency 5`。
- **Smoke / 微基准**（长训前可选）：`python -m ...verify_p2d`；`python -m ..._bench_a2d --batch_size 64 --seq_len 60`。
...
batch_size=64
save_frequency=5          # 历史实验（H1/A2/A2b/A2c）命令中显式传入
nr_epochs=100
```

**2026-05-24 起**：仓库默认 **`save_frequency=10`**（`config_constraint_fused_high_modify.py`）。**A2d** 等新实验可不传该参数；历史对照实验仍按原命令 `save_frequency=5` 解读。**A2d** 额外约定：`geom_horizontal` 等监控列仅在 `step % log_frequency==0` 写入 CSV（P2d-03）；A2d 开关开启时另增 8 列 `geom_h_loss`…`aux_gamma_perp`。
...
| A2d corrected interpreter Python 循环导致单 epoch 极慢 | 300 epoch 长训不可排期 | **P2d-02b 已修复**：向量化 + BCE 合批 + metric 懒求值；微基准 ~388×；**须 `--dataset_cache disk`** 避免重复 CPU 约束提取；见 §3.2d(5) |
...
**A2d 的定位**：A2c 已闭合「主路径 + mild 辅助」，但实测显示 `parallel` 卡在 0.84，`geom_para` 已饱和到 0.011 而 `pair_recon_loss` 已压到 0.026，仍与 DeepCADOrigin@1000 的 0.8617 差约 1.9 pt。A2d 针对此平台做四项联合改造（**interpreter 起点修正 + `geom_loss` 双向 hard BCE + `γ` 四路拆分（`γ_para=3`）+ 长训 300 epoch 三段调度**）。**P2d-01/02 功能改造与 smoke 已通过；P2d-03 性能优化已落地（2026-05-24），长训可启动**。若 A2d@200 越过 DeepCADOrigin@1000 的 `parallel`=0.8617，则**成为论文主推配置**，A2c 退为消融对照；若 A2d 仍未突破，则需要回到结构层面（A3/A4 单因素 / A7 / A8）或 T 系列（`ConstraintMetricCore`）监督对齐线。
