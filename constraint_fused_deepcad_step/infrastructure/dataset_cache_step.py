...
def resolve_dataset_cache_dir(config, phase: str) -> str:
    ...
    return os.path.join(data_root, ".cache", "deepcad_step", cache_key, phase)
...
def build_source_stat(h5_path: str) -> Optional[Dict[str, int]]:
    try:
        st = os.stat(h5_path)
    except OSError:
        return None
...
def source_stat_matches(stored: Optional[Dict[str, Any]], current: Optional[Dict[str, int]]) -> bool:
...
def load_cached_sample(
    ...
    *,
    clone: bool = True,
    current_source_stat: Optional[Dict[str, int]] = None,
...
def save_cached_sample(path: str, sample: Dict[str, Any], source_stat: Optional[Dict[str, int]]) -> None:
