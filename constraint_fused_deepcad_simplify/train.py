...
from constraint_fused_deepcad_simplify.application.evaluate_axis_constraints import resolve_device
...
def main() -> None:
    cfg = ConfigConstraintFusedSimplify("train")
    ...
    train_loader = get_simplify_dataloader("train", cfg, shuffle=True)
    val_loader = cycle(get_simplify_dataloader("validation", cfg, shuffle=False))
    ...
    for epoch in range(clock.epoch, cfg.nr_epochs + 1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if clock.step % cfg.val_frequency == 0:
                ...
                for key in ("loss", "loss_cmd", "axis_loss"):
