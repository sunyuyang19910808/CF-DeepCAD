...
from constraint_fused_deepcad_simplify.domain.services import iter_line_command_positions
from constraint_fused_deepcad_simplify.infrastructure.repository import CadVectorRepository
from constraint_fused_deepcad_simplify.sketch_preparation.batch_assembler_simplify import ConstraintBatchAssemblerSimplify
from constraint_fused_deepcad_simplify.sketch_preparation.constraint_extractor_simplify import ConstraintExtractorSimplify


class CADDatasetSimplify(Dataset):
    def __init__(self, phase: str, config):
        ...
        self.extractor = ConstraintExtractorSimplify(
            angle_thresh=config.angle_thresh,
            grid_size=config.grid_size,
        )
        self.assembler = ConstraintBatchAssemblerSimplify(
...
def fused_simplify_collate_fn(batch):
    return {
...
def get_simplify_dataloader(phase: str, config, shuffle=None):
    is_shuffle = phase == "train" if shuffle is None else shuffle
    dataset = CADDatasetSimplify(phase, config)
    return DataLoader(
        dataset,
        batch_size=config.batch_size,
        shuffle=is_shuffle,
        num_workers=config.num_workers,
        collate_fn=fused_simplify_collate_fn,
    )
