...
from constraint_fused_deepcad_simplify_modify2.domain.services import iter_line_command_positions
from constraint_fused_deepcad_simplify_modify2.infrastructure.repository import CadVectorRepository
from constraint_fused_deepcad_simplify_modify2.sketch_preparation.batch_assembler_simplify_modify2 import (
    ConstraintBatchAssemblerSimplifyModify2,
)
...
class CADDatasetSimplifyModify2(Dataset):
    def __init__(self, phase: str, config):
        ...
        self.extractor = ConstraintExtractorSimplifyModify2(
            angle_thresh=config.angle_thresh,
            dist_thresh=config.dist_thresh,
            grid_size=config.grid_size,
        )
        self.assembler = ConstraintBatchAssemblerSimplifyModify2(
...
def fused_simplify_modify2_collate_fn(batch):
    return {
...
def get_simplify_modify2_dataloader(phase: str, config, shuffle=None):
    is_shuffle = phase == "train" if shuffle is None else shuffle
    dataset = CADDatasetSimplifyModify2(phase, config)
    return DataLoader(
        dataset,
        batch_size=config.batch_size,
        shuffle=is_shuffle,
        num_workers=config.num_workers,
        collate_fn=fused_simplify_modify2_collate_fn,
    )
