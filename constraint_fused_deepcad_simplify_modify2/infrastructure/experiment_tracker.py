...
class ExperimentTracker:
    def __init__(self, artifact_dir: str, exp_id: str, data_root: str):
    ...
    def write_manifest(self, config_snapshot: dict, dataset_name: str = "deepcad", dataset_split: str = "train"):
        ...
        with open(os.path.join(self.artifact_dir, "manifest.json"), "w", encoding="utf-8") as file_obj:
            json.dump(manifest, file_obj, indent=2, ensure_ascii=False)

    def append_train_metrics(self, row: dict):
        path = os.path.join(self.artifact_dir, "train_metrics.csv")
    ...
    def write_eval_metrics(self, metrics: dict):
        with open(os.path.join(self.artifact_dir, "eval_metrics.json"), "w", encoding="utf-8") as file_obj:
            json.dump(metrics, file_obj, indent=2, ensure_ascii=False)

    def write_best_checkpoint(self, path: str):
        with open(os.path.join(self.artifact_dir, "best_checkpoint.txt"), "w", encoding="utf-8") as file_obj:
            file_obj.write(path + "\n")

    def write_qualitative_cases(self, cases: list):
        with open(os.path.join(self.artifact_dir, "qualitative_cases.json"), "w", encoding="utf-8") as file_obj:
            json.dump(cases, file_obj, indent=2, ensure_ascii=False)
