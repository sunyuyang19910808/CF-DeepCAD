...
from constraint_fused_deepcad_simplify_modify2.domain.entities import (
    ConstraintRelation,
    ConstraintType,
...
class ConstraintBatchAssemblerSimplifyModify2:
        ...
        for rel in relations:
            ...
            for line_idx in {rel.line_a, rel.line_b}:
                if line_idx >= len(line_positions):
                    raise ValueError(
                        "Constraint line {} exceeds discovered line positions {}.".format(
                            line_idx, len(line_positions)
                        )
                    )
