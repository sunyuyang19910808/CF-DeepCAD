...
from constraint_fused_deepcad_simplify_modify2_low_risk.application.generate_use_case import resolve_device
...
from constraint_fused_deepcad_simplify_modify2_low_risk.infrastructure.checkpoint_repository_fs import (
...
from constraint_fused_deepcad_simplify_modify2_low_risk.infrastructure.experiment_tracker import ExperimentTracker
...
def main() -> None:
    cfg = ConfigConstraintFusedSimplifyModify2LowRisk("train")
    ...
    train_loader = get_simplify_modify2_low_risk_dataloader("train", cfg, shuffle=True)
    val_loader = cycle(get_simplify_modify2_low_risk_dataloader("validation", cfg, shuffle=False))
    ...
    for epoch in range(clock.epoch, cfg.nr_epochs + 1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if clock.step % cfg.val_frequency == 0:
                ...
                for key in ("loss", "loss_cmd", "pred_loss", "recon_loss", "geom_loss"):
